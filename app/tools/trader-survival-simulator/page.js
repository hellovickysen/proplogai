'use client';

import Link from 'next/link';
import LandingFooter from '@/components/landing/LandingFooter';
import LandingNav from '@/components/landing/LandingNav';
import TraderSurvivalSimulator from '@/components/tools/TraderSurvivalSimulator';

export default function PublicTraderSurvivalSimulatorPage() {
  return (
    <>
      <LandingNav />
      <main className="min-h-screen bg-[#07070b]">
        <div className="mx-auto max-w-6xl p-4 md:p-8">
          <Link
            href="/tools"
            className="inline-flex items-center gap-1.5 font-mono text-xs uppercase tracking-wider text-white/55 transition-colors hover:text-white/90"
          >
            ← Tools
          </Link>
          <div className="mt-4">
            <TraderSurvivalSimulator />
          </div>
        </div>
      </main>
      <LandingFooter />
    </>
  );
}

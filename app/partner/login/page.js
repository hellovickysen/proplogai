 ...
 * Supports Google (for accounts created via Google in the main app) and
 * email/password. On success, cookies scope to the partner subdomain and the
 * user lands on the affiliate dashboard.
 */
  ...
  async function onGoogle() {
    ...
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          // On the partner host this resolves to
          // https://partner.proplogai.com/auth/callback (a middleware passthrough).
  ...
  return (
    <div className="mx-auto flex min-h-[70vh] max-w-md flex-col justify-center px-5 py-16">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
        ...
        {/* Google — required for accounts created with Google in the main app */}
        <button
          type="button"
          onClick={onGoogle}
          disabled={googleBusy}

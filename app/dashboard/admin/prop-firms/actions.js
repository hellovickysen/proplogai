...
export async function addFirm(adminEmail, firm) {
  ...
  const { data, error } = await db
    ...
    .insert({
      name: firm.name,
      slug,
      profit_target: firm.profit_target !== '' ? Number(firm.profit_target) : 8,
      daily_drawdown: firm.daily_drawdown !== '' ? Number(firm.daily_drawdown) : 2,
      overall_drawdown: firm.overall_drawdown !== '' ? Number(firm.overall_drawdown) : 4,
      min_trading_days: firm.min_trading_days !== '' ? Number(firm.min_trading_days) : 5,
      challenge_days: firm.challenge_days !== '' ? Number(firm.challenge_days) : 30,
      max_risk_per_trade: firm.max_risk_per_trade !== '' ? Number(firm.max_risk_per_trade) : null,

...
- Phase O: Propol AI Trading Performance Coach — Fixed AI error (model slug anthropic/claude-3-5-haiku → anthropic/claude-haiku-4.5), built guardrails engine (lib/guardrails.js with HARD_BLOCK patterns for financial advice + SOFT_REWRITE rules + post-response scanning), revamped AI prompts with 4-module coaching architecture (Discipline, Psychology, Performance, Reflection), created trade context helper (lib/tradeContext.js — getUserTradeContext builds rich context from user setups/stats/emotions/patterns), updated rate limits (Basic 5 analyses/month + 1 review; Elite 100/month + 4 reviews), redesigned /coach into Propol AI Coach Hub with 4 tabs (Overview with score cards + sparklines, Trade Analysis with expandable cards, Monthly Review with 5-section reports, Insights with pattern cards), coach reports now INSERT for history tracking, Propol branding + no-financial-advice disclaimer across landing page/coach hub/terms. New components: PropolCoachHub.js, CoachTabs.js, OverviewTab.js, TradeAnalysisTab.js, MonthlyReviewTab.js, InsightsTab.js. JSON repair in extractJson for truncated AI responses.

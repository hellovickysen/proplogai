...
- lib/: ai.js (OpenRouter + 25s timeout + prompt injection defense), email.js (Resend + HTML escaping), stats.js (computeStats, equitySeries, equityChartData, fmtMoney, fmtR), discipline.js (computeDisciplineStats, computeWeeklyScore, computeEliteWeekStreak, calculateWeekScore), achievements.js (ACHIEVEMENT_DEFS, computeAchievements), quotes.js, emotions.js, security.js (isDisposableEmail with subdomain check, validatePassword), notifications.js (notify, notifyAdmin, TYPES), imageUtils.js (processImageFile, pdf.js with retry), supabase/ (client.js, server.js, admin.js)
- middleware.js: Auth session refresh + /dashboard + /admin guard (with error redirect to /login)
- supabase/migrations/: SQL migration files 0001-0023

## Database Schema (14 tables + 2 functions + 1 trigger, all RLS-scoped)
...
- journal_entries: id, user_id, trade_id (CASCADE), note, emotions[], confidence, screenshot_url, screenshot_urls (jsonb), created_at
- ai_insights: id, user_id, trade_id, type ('trade_analysis' or 'coach_report'), summary, mistakes (jsonb), severity, created_at
- user_preferences: id, user_id (UNIQUE), avatar_url, full_name (text, migration 0023), custom_emotions[], custom_setups[], default_confidence, onboarding_complete, share_code (unique), show_calendar, show_trades, show_payouts, show_trophies, calendar_mode (fixed/rolling), calendar_start, calendar_end, calendar_rolling_days, referral_balance (numeric), referred_by, created_at, updated_at
- subscriptions: id, user_id, plan ('free'/'pro'/'elite'), status, stripe_id, renews_at, created_at
...
- support_tickets: id, user_id, user_email, category (bug/platform_issue/feature_request/general_support/account_billing), subject, description, screenshot_url, screenshot_urls (jsonb), status (open/in_progress/resolved/closed), admin_reply, created_at, updated_at
...
Auth (email/password + Google OAuth with automatic account linking), email verification (Supabase confirm email + custom SMTP via Resend), disposable email blocking (200+ domains + subdomain check), password strength meter, onboarding, trade logging (multi-setup up to 5 with per-setup follow tracking), journal entries, AI trade analysis, AI coach report with rulebook discipline, P&L calendar (v5, dual-render desktop/mobile with weekends toggle), dashboard with stats/equity curve/interactive chart/discipline cards/weekly score ring/achievement badges, trade filtering, shareable P&L cards (3 aspect ratios: Story 9:16, Square 1:1 for Twitter/X at 480x480, Landscape 16:9), shareable trade journals (24h auto-expiry), settings with public profile config + full name editing + walkthrough replay, admin panel (overview with SVG trend charts/users with full name display and search/AI usage/revenue/tickets), CSV export, email coach report (Resend with HTML-escaped AI fields), notification system (13 triggers, bell with polling, separate admin stream), toast notifications (color-coded: green add/edit, red delete with trash icon), delete confirmation dialog (styled ConfirmDialog with a11y across entire app), loading skeletons for all dashboard routes, page transitions, mobile responsive (verified 375px + 320px), empty states, custom 404, error boundary (hides raw errors in production), OG meta, risk disclaimer footer, privacy policy, terms of service (13 sections including referral credits policy), rulebook (setup CRUD with 8 defaults including Good SL), prop firm expense tracker (3-tab: dashboard/accounts/payouts + individual firm dashboards with edit/delete/inline rename, sorting by Recent/Highest/A-Z, Futures/CFD tags and filter), trophy wall (upload/gallery/lightbox/share), public profile (single-scroll with calendar/trades/payouts/certificates), referral system (unique links, $1 reward via DB trigger, referral dashboard with credit usage policy), support ticket system (5 categories, multi-image upload, user ticket history with detail views, admin ticket management with filters/status/reply), password eye toggle (login + settings), collapsible desktop sidebar (60px icon rail default, 200px expanded, localStorage persistence), mobile nav slide-from-left drawer, beta notice banner (one-time dismissible), landing nav redesign (Lyrafin-inspired desktop pill buttons + mobile hamburger overlay with a11y), cookie consent banner (localStorage persist, brand gradient accept button), full name support (auto-fetch from Google OAuth, editable in Settings, displayed in Sidebar/MobileNav/Admin with avatar initial), dashboard share button always visible (Total P&L fallback when no today trades, removed Avg R stat chip), guided tooltip walkthrough for new users (7-step GuidedTour with spotlight overlay, auto-triggers on 0 trades, replayable from Settings, data-tour attributes on key UI elements)
...
## Completed Work (Phases 1-8 + Phases G-K)

...
- supabase/migrations/: SQL migration files 0001-0027
...
- support_tickets: id, user_id, user_email, category (bug/platform_issue/feature_request/general_support/account_billing), subject, description, screenshot_url, screenshot_urls (jsonb), status (open/in_progress), reply_count (int), created_at, updated_at
...
- Phase N: Ticket system restructure + image compression consistency — Ticket conversation system (migration 0027: ticket_replies table with CASCADE delete, RLS policies, user update/delete policies on support_tickets, dropped admin_reply column, added reply_count), multi-reply by both user and admin with message bubbles (user=violet, admin=cyan), close ticket by either party with optional transcript email (unchecked by default), multi-select bulk delete (no transcript), notification types added (TICKET_USER_REPLIED, TICKET_CLOSED, TICKET_STATUS), admin notification types list updated in both dashboard and admin layouts, skip notifyAdmin when user IS admin (prevents duplicates), statuses simplified to open/in_progress only (close = delete), screenshot cleanup on close/delete, WebP compression (processImageFile) added to support ticket screenshot uploads and avatar uploads (all 4 upload points now consistent), sendTicketTranscript email function in lib/email.js

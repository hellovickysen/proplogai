                ...
                return (
                  <tr key={'week-' + wi}>
                    {week.map((cell, di) => {
                      ...
                      let bgStyle = {};
                      if (e) {
                        bgStyle = { backgroundColor: e.net >= 0 ? (isOverflow ? 'rgba(34, 197, 94, 0.06)' : 'rgba(34, 197, 94, 0.15)') : (isOverflow ? 'rgba(239, 68, 68, 0.07)' : 'rgba(239, 68, 68, 0.18)') };
                      }
                      if (isOverflow) {
                        bgStyle.backgroundImage = 'repeating-linear-gradient(-45deg, rgba(255,255,255,0.025) 0, rgba(255,255,255,0.025) 1px, transparent 1px, transparent 10px)';
                      }
                      ...
                      if (isSaturday) {
                        const satContent = (
                          <div
                            className={'relative flex h-28 flex-col rounded-lg overflow-hidden transition-all duration-200 ' + todayBorder + ' ' + (ws.count === 0 ? 'opacity-25' : '') + (isOverflow ? ' opacity-60' : '') + (isSel ? ' ring-1 ring-inset ring-cyan-400/50' : '') + (e ? ' group-hover:-translate-y-0.5 group-hover:border-cyan-400/60 group-hover:shadow-lg group-hover:shadow-cyan-500/10' : '')}
                            style={bgStyle}
                          >
                      ...
                      const cellContent = (
                        <div
                          className={'relative flex h-28 flex-col rounded-lg overflow-hidden transition-all duration-200 ' + todayBorder + ' ' + (isOverflow ? 'opacity-60' : '') + (isSel ? ' ring-1 ring-inset ring-cyan-400/50' : '') + (e ? ' cursor-pointer group-hover:-translate-y-0.5 group-hover:border-cyan-400/60 group-hover:shadow-lg group-hover:shadow-cyan-500/10' : '')}
                          style={bgStyle}
                        >
                          <div className="flex items-center gap-1 px-2 pt-1.5">
                            <span className={dayNumClass(isToday, isOverflow)}>{d}</span>
            ...
            return (
              ...
              >
                {filtered.map((cell, di) => {
                  ...
                  const cellContent = (
                    <div
                      className={'relative flex h-[72px] flex-col transition-all duration-200 ' + (isOverflow ? 'opacity-60' : '') + (isSel ? ' ring-2 ring-inset ring-cyan-400/50' : '') + (e ? ' cursor-pointer group-hover:-translate-y-0.5 group-hover:ring-1 group-hover:ring-cyan-400/60 group-hover:shadow-lg group-hover:shadow-cyan-500/10' : '')}
                      style={bgStyle}
                    >

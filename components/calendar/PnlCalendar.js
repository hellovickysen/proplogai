...
export default function PnlCalendar({ trades, monthPnl }) {
  ...
  const mobileCells = showWeekends
    ? allCells
    : allCells.filter((c) => c.dow !== 0 && c.dow !== 6);
  const mobileCols = showWeekends ? 7 : 5;
  const mobileDow = showWeekends ? DOW_ALL : DOW_WEEKDAY;
  ...
  return (
    <div>
      ...
      {/* ══════════════════════════════════════════════════════════
          DESKTOP — original 7-col grid, no toggle
          ══════════════════════════════════════════════════════════ */}
      <div className="hidden sm:block">
        ...
        <div className="grid grid-cols-7 gap-1">
          {allCells.map((cell, i) => {
            ...
            const pnl = !isOverflow && byDay[d] ? byDay[d].net : undefined;
      ...
      {/* ══════════════════════════════════════════════════════════
          MOBILE — 5-col grid with toggle, bigger cells, trade count
          ══════════════════════════════════════════════════════════ */}
      <div className="sm:hidden">
        ...
        {/* day grid */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(' + mobileCols + ', 1fr)', gap: '4px' }}>

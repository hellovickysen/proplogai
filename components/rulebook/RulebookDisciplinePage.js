...
function ToggleRule({ rule, stored, accountId, custom = false }) {
  ...
  return <div className="relative flex items-center justify-between gap-4 rounded-xl border border-white/10 bg-white/[0.03] p-4"><StatusLight active={enabled} className="absolute right-3 top-3" /><div className="min-w-0 flex-1"><h3 className="font-display text-sm font-semibold">{rule.title}</h3><p className="mt-1 text-xs leading-relaxed text-white/45">{rule.description}</p><Status saved={saved} /></div><RuleToggle enabled={enabled} onToggle={toggle} saving={saving} /></div>;
}

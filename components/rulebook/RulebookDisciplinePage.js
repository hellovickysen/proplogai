...
function ToggleRule({ rule, stored, accountId, custom = false }) {
  const [enabled, setEnabled] = useState(stored ? stored.enabled : true);
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  const router = useRouter();
  ...
  return <div className="flex items-center justify-between gap-4 rounded-xl border border-white/10 bg-white/[0.03] p-4"><div><h3 className="font-display text-sm font-semibold">{rule.title}</h3><p className="mt-1 text-xs leading-relaxed text-white/45">{rule.description}</p><Status saved={saved} /></div><button type="button" onClick={toggle} disabled={saving} aria-pressed={enabled} className={'relative h-7 w-12 shrink-0 rounded-full transition-colors ' + (enabled ? 'bg-cyan-400' : 'bg-white/10')}><span className={'absolute top-1 h-5 w-5 rounded-full bg-[#08080f] transition-transform ' + (enabled ? 'translate-x-6' : 'translate-x-1')} /></button></div>;
}
...
function SegmentedRule({ rule, stored, accountId }) {
  ...
  async function choose(next) {
    setValue(next);
    const result = await saveDisciplineRule({ account_id: accountId, category: 'condition', rule_key: rule.key, rule_type: rule.key, title: rule.title, description: rule.description, value: next, unit: null, enabled: true, sort_order: 1 });
    if (!result.error) { setSaved(true); setTimeout(() => setSaved(false), 1200); router.refresh(); }
  }
  return <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4"><h3 className="font-display text-base font-semibold">{rule.title}</h3><p className="mt-1 text-xs leading-relaxed text-white/45">{rule.description}</p><div className="mt-3 flex flex-wrap gap-1.5">{rule.options.map(([key, label]) => <button key={key} type="button" onClick={() => choose(key)} className={'rounded-lg border px-3 py-2 text-xs font-semibold ' + (value === key ? 'border-cyan-400/50 bg-cyan-500/15 text-cyan-300' : 'border-white/10 bg-black/20 text-white/50')}>{label}</button>)}</div><Status saved={saved} /></div>;
}
...
function TradingHours({ stored, accountId }) {
  ...
  const [start, setStart] = useState(initial.start || '09:30');
  const [end, setEnd] = useState(initial.end || '12:00');
  const [saved, setSaved] = useState(false);
  const router = useRouter();
  async function save(nextEnabled = enabled) {
    setEnabled(nextEnabled);
    const result = await saveDisciplineRule({ account_id: accountId, category: 'condition', rule_key: 'trading_hours', rule_type: 'trading_hours', title: 'Trading Hours', description: 'I only trade during my chosen trading window.', value: start + '–' + end, unit: null, enabled: nextEnabled, metadata: { start, end }, sort_order: 1 });
    if (!result.error) { setSaved(true); setTimeout(() => setSaved(false), 1200); router.refresh(); }
  }
  return <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4"><div className="flex items-center justify-between gap-3"><div><h3 className="font-display text-base font-semibold">Trading Hours</h3><p className="mt-1 text-xs text-white/45">I only trade during my chosen trading window.</p></div><button type="button" onClick={() => save(!enabled)} aria-pressed={enabled} className={'relative h-7 w-12 shrink-0 rounded-full ' + (enabled ? 'bg-cyan-400' : 'bg-white/10')}><span className={'absolute top-1 h-5 w-5 rounded-full bg-[#08080f] transition-transform ' + (enabled ? 'translate-x-6' : 'translate-x-1')} /></button></div><div className="mt-3 flex items-center gap-2"><input type="time" value={start} onChange={(event) => setStart(event.target.value)} className="rounded-lg border border-white/10 bg-black/30 px-2 py-1.5 text-xs" /><span className="text-white/35">to</span><input type="time" value={end} onChange={(event) => setEnd(event.target.value)} className="rounded-lg border border-white/10 bg-black/30 px-2 py-1.5 text-xs" /><button type="button" onClick={() => save()} className="ml-auto text-xs font-semibold text-cyan-300">Save</button></div><Status saved={saved} /></div>;
}
...
export default function RulebookDisciplinePage({ rules = [], scopeAccount = null }) {
  ...
  const enabledBehaviors = BEHAVIORS.filter((rule) => stored[rule.key] ? stored[rule.key].enabled : true).length;
  const response = stored.rule_break_response?.value || 'review_first';

  async function chooseResponse(value) { await saveDisciplineRule({ account_id: scopeAccount?.id || null, category: 'response', rule_key: 'rule_break_response', rule_type: 'rule_break_response', title: 'When I Break a Rule', description: 'Decide what happens after I violate one of my own rules.', value, unit: null, enabled: true, sort_order: 1 }); router.refresh(); }
  ...
  return <div className="px-4 py-8 sm:px-6"><div className="mb-7 flex flex-wrap items-start justify-between gap-3"><div className="max-w-2xl"><span className="font-mono text-xs uppercase tracking-wider text-cyan-300">Your discipline foundation</span><h1 className="mt-2 font-display text-3xl font-bold">Rulebook</h1><p className="mt-2 text-sm leading-relaxed text-white/55">These are the rules I choose to follow before and during trading. They are personal boundaries, not trading recommendations.</p></div><div className="rounded-full border border-emerald-400/25 bg-emerald-500/10 px-3 py-1.5 text-xs font-semibold text-emerald-300">● Rulebook Active</div></div><div className="mb-6 flex items-center gap-2 font-mono text-xs text-white/45"><span className="rounded-full border border-white/10 bg-white/[0.03] px-2.5 py-1">{scopeLabel}</span><span>{NON_NEGOTIABLES.length + enabledBehaviors} active rules</span></div>
    ...
    <section className="mt-9"><h2 className="font-display text-xl font-semibold">4. When I Break a Rule</h2><p className="mt-1 text-sm text-white/45">Decide what happens when I violate one of my own rules.</p><div className="mt-4 grid gap-3 sm:grid-cols-3">{[['stop_trading', 'Stop Trading', 'Stop trading for the day.'], ['take_a_break', 'Take a Break', 'Pause for 30 minutes before deciding whether to continue.'], ['review_first', 'Review First', 'Review the trade before taking another position.']].map(([key, title, description]) => <button key={key} type="button" onClick={() => chooseResponse(key)} className={'rounded-xl border p-4 text-left transition-colors ' + (response === key ? 'border-cyan-400/50 bg-cyan-500/10' : 'border-white/10 bg-white/[0.03] hover:bg-white/[0.05]')}><div className="font-display text-sm font-semibold">{title}</div><p className="mt-1 text-xs leading-relaxed text-white/45">{description}</p></button>)}</div></section>

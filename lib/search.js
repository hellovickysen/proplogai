...
async function searchJournal(supabase, userId, query, filters = {}) {
  // Journal search needs a join-like approach — search notes/lessons
  let q = supabase
  ...
  if (query && !filters.emotion && !filters.tag) {
    // Supabase doesn't support OR across columns easily with ilike,
    // so we search note first
    q = q.or(`note.ilike.%${query}%,lesson.ilike.%${query}%`);
  }

  const { data, error } = await q;
  if (error) { console.error('searchJournal error:', error); return []; }
  return (data || []).map(j => ({
    type: 'journal',
    id: j.id,
    tradeId: j.trade_id,

...
async function searchTrades(supabase, userId, query, filters = {}) {
  ...
  let q = supabase
    .from('trades')
    .select('id, pair, direction, pnl, trade_date, setup, session, timeframe, lot_size, entry_price, exit_price, stop_loss, created_at')
    .eq('user_id', userId)
...
async function searchTradesByEmotion(supabase, userId, emotion, filters = {}) {
  ...
  let tq = supabase
    .from('trades')
    .select('id, pair, direction, pnl, trade_date, setup, session, created_at')
    .eq('user_id', userId)

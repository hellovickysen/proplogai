...
export async function finishGoogleDriveConnection(supabase, user, code, state) {
  ...
  const accountResponse = await fetch('https://openidconnect.googleapis.com/v1/userinfo', { headers: { Authorization: `Bearer ${tokens.access_token}` } });
  ...
  const { error } = await supabase.from('backup_drive_connections').upsert({
    ...
    revoked_at: null,
    provider_email: account?.email || null,
  }, { onConflict: 'user_id' });
...
async function accessToken(connection) {
  ...
  if (!grantedScopes.includes('https://www.googleapis.com/auth/drive.file')) {
    throw new Error('Google Drive permission changed. Reconnect Google Drive to enable the visible backup folder.');
  }
...
async function ensureVisibleBackupFolder(supabase, userId, connection, validAccessToken) {
  if (connection.provider_folder_id) {
    const existing = await fetch(`${GOOGLE_DRIVE_FILES_URL}/${encodeURIComponent(connection.provider_folder_id)}?fields=id,trashed`, { headers: { Authorization: `Bearer ${validAccessToken}` } });
    const metadata = existing.ok ? await existing.json() : null;
    if (metadata?.id && !metadata.trashed) return connection.provider_folder_id;
  }
  const response = await fetch(GOOGLE_DRIVE_FILES_URL, { method: 'POST', headers: { Authorization: `Bearer ${validAccessToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ name: 'PropLogAI Backups', mimeType: 'application/vnd.google-apps.folder' }) });
  const folder = await response.json();
  if (!response.ok || !folder.id) throw new Error('Google Drive could not create your PropLogAI Backups folder.');
  const { error } = await supabase.from('backup_drive_connections').update({ provider_folder_id: folder.id, updated_at: new Date().toISOString() }).eq('user_id', userId);
  if (error) throw new Error('Unable to save your Google Drive backup folder.');
...
export async function uploadGoogleDriveBackup(supabase, userId, connection, archive, filename) {
  const { accessToken: validAccessToken, tokens } = await accessToken(connection);
  await persistTokens(supabase, userId, tokens);
  const folderId = await ensureVisibleBackupFolder(supabase, userId, connection, validAccessToken);
  const boundary = `proplogai-${randomBytes(12).toString('hex')}`;
  const metadata = Buffer.from(JSON.stringify({ name: filename, parents: [folderId], mimeType: 'application/zip' }));
  const body = Buffer.concat([
    Buffer.from(`--${boundary}\r\nContent-Type: application/json; charset=UTF-8\r\n\r\n`), metadata,
    Buffer.from(`\r\n--${boundary}\r\nContent-Type: application/zip\r\n\r\n`), archive,
    Buffer.from(`\r\n--${boundary}--`),
  ]);
  const response = await fetch(GOOGLE_DRIVE_UPLOAD_URL, { method: 'POST', headers: { Authorization: `Bearer ${validAccessToken}`, 'Content-Type': `multipart/related; boundary=${boundary}` }, body });
  const payload = await response.json();
  if (!response.ok || !payload.id) throw new Error('Google Drive could not save this backup.');
  return { providerFileId: payload.id, folderId };
}

export async function downloadGoogleDriveBackup(supabase, userId, connection, providerFileId) {
  const { accessToken: validAccessToken, tokens } = await accessToken(connection);
  await persistTokens(supabase, userId, tokens);
  const response = await fetch(`${GOOGLE_DRIVE_FILES_URL}/${encodeURIComponent(providerFileId)}?alt=media`, { headers: { Authorization: `Bearer ${validAccessToken}` } });
...
export async function reconcileGoogleDriveBackups(supabase, userId, connection) {
  const { data: runs, error } = await supabase.from('backup_runs').select('id, provider_file_id, completed_at').eq('user_id', userId).eq('destination', 'google_drive').eq('status', 'completed').not('provider_file_id', 'is', null).order('completed_at', { ascending: false });
  if (error) throw new Error('Unable to read cloud backup status.');
  const { accessToken: validAccessToken } = await accessToken(connection);
  const available = [];
  for (const run of runs || []) {
    const response = await fetch(`${GOOGLE_DRIVE_FILES_URL}/${encodeURIComponent(run.provider_file_id)}?fields=id,trashed`, { headers: { Authorization: `Bearer ${validAccessToken}` } });
    const metadata = response.ok ? await response.json() : null;
    if (metadata?.id && !metadata.trashed) available.push(run);
    else await supabase.from('backup_runs').update({ provider_file_id: null }).eq('id', run.id).eq('user_id', userId);
  }
  return { count: available.length, latestCompletedAt: available[0]?.completed_at || null };
}
...
export async function retainLatestGoogleDriveBackups(supabase, userId, connection, keep = 7) {
  ...
  for (const run of runs.slice(keep)) {
    const response = await fetch(`${GOOGLE_DRIVE_FILES_URL}/${encodeURIComponent(run.provider_file_id)}`, { method: 'DELETE', headers: { Authorization: `Bearer ${validAccessToken}` } });

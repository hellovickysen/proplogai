...
export async function finishGoogleDriveConnection(supabase, user, code, state) {
  ...
  const accountResponse = await fetch(`${GOOGLE_DRIVE_FILES_URL}/about?fields=user(emailAddress)`, { headers: { Authorization: `Bearer ${tokens.access_token}` } });
  ...
  const { error } = await supabase.from('backup_drive_connections').upsert({
    ...
    provider_email: account?.user?.emailAddress || null,
  }, { onConflict: 'user_id' });

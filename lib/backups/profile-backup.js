...
function archiveSigningKey() {
  const key = process.env.BACKUP_ARCHIVE_SIGNING_KEY;
  if (!key || key.length < 32) throw new Error('Secure archive ownership validation is not configured yet.');
  return key;
}

function archiveOwnerProof(userId) {
  return createHmac('sha256', archiveSigningKey())
    .update(`${BACKUP_FORMAT}:${BACKUP_VERSION}:${userId}`)
    .digest('hex');
}

function assertArchiveOwner(manifest, preferences, userId) {
  const ownerId = preferences?.user_id;
  const proof = manifest?.ownerProof;
  if (!ownerId || !proof || typeof proof !== 'string') {
    throw new Error('This backup was created before owner validation. Download a new backup from this account.');
  }
  const expected = archiveOwnerProof(ownerId);
  const validProof = proof.length === expected.length
    && timingSafeEqual(Buffer.from(proof, 'utf8'), Buffer.from(expected, 'utf8'));
  if (!validProof) throw new Error('This backup could not be validated. Download a new backup and try again.');
  if (ownerId !== userId) throw new Error('This backup belongs to another account and cannot be restored here.');
}
...
const USER_PREFERENCES_COLUMNS = 'user_id,avatar_url,custom_emotions,default_confidence,custom_setups,calendar_mode,calendar_start,calendar_end,calendar_rolling_days,show_calendar,show_payouts,show_trophies,show_trades,full_name,custom_tags,active_account_id';

const IMPORT_ORDER = ['accounts', 'setups', 'trades', 'journal_entries', 'ai_insights', 'expenses', 'payouts', 'trophies'];
...
async function exportAssets(supabase, references) {
  ...
  for (const asset of references) {
    ...
    if (error || !data) {
      unavailable.push({ bucket: asset.bucket, path: asset.path, reason: error?.message || 'File is unavailable.' });
      continue;
    }
    const content = Buffer.from(await data.arrayBuffer());
    if (content.length > BACKUP_LIMITS.maxEntryBytes) {
      unavailable.push({ bucket: asset.bucket, path: asset.path, reason: 'File exceeds the 10MB attachment limit.' });
      continue;
    }
  ...
  return { entries, manifest, unavailable };
}
...
export async function createProfileBackup(supabase, user) {
  ...
  const { entries: assetEntries, manifest: assets, unavailable } = await exportAssets(supabase, collectAssetReferences(preferences, records));
  ...
  const manifest = {
    ...
    source: 'PropLogAI',
    ownerProof: archiveOwnerProof(user.id),
...
function ensureAllowedArchive(files, manifest) {
  ...
  for (const [path, content] of files.entries()) {
    ...
    // A manifest cannot contain a checksum of itself without recursive mutation.
    // Its structure is validated above; every data and asset entry is checksummed below.
...
async function restoreAssets(supabase, userId, inspected, sameAccount = false) {
  ...
  for (const asset of inspected.manifest.assets || []) {
    ...
    const destination = sameAccount && asset.path.startsWith(`${userId}/`)
      ? asset.path
      : `${userId}/restored/${inspected.manifest.exportedAt.replace(/[^0-9]/g, '').slice(0, 14)}/${asset.path.replace(/[^a-zA-Z0-9_./-]/g, '_')}`;
...
async function saveMappings(supabase, rows) {
  if (rows.length === 0) return;
  const { error } = await supabase.from('backup_import_mappings').upsert(rows, { onConflict: 'user_id,source_table,source_id' });
  if (error) throw new Error(`Unable to save import mappings: ${error.message}`);
}

const FINGERPRINT_EXCLUDED_FIELDS = new Set([
  'id', 'user_id', 'created_at', 'updated_at', 'completed_at', 'calculated_at', 'evaluated_at',
  'share_id', 'shared_until', 'avatar_url', 'file_url', 'screenshot_url', 'screenshot_urls', 'reference_images',
]);
...
function canonicalizeForFingerprint(value) {
  ...
  if (value && typeof value === 'object') {
    ...
    for (const key of Object.keys(value).sort()) {
      if (!FINGERPRINT_EXCLUDED_FIELDS.has(key)) output[key] = canonicalizeForFingerprint(value[key]);
    }
    return output;
  }
  return value;
...
function recordFingerprint(row) {
  return sha256(JSON.stringify(canonicalizeForFingerprint(row)));
}

async function loadExistingIndexes(supabase, userId) {
  const entries = await Promise.all(TABLE_SPECS.map(async (spec) => {
    const rows = await readAllRows(supabase, spec, userId);
    const byId = new Map();
    const byFingerprint = new Map();
    for (const row of rows) {
      if (row.id) byId.set(row.id, row.id);
      const fingerprint = recordFingerprint(row);
      if (!byFingerprint.has(fingerprint)) byFingerprint.set(fingerprint, row.id);
    }
    return [spec.file, { byId, byFingerprint }];
  }));
  return Object.fromEntries(entries);
}

async function loadExistingMappings(supabase, userId) {
  const { data, error } = await supabase
    .from('backup_import_mappings')
    .select('source_table, source_id, destination_id')
    .eq('user_id', userId);
  if (error) throw new Error(`Unable to load prior import mappings: ${error.message}`);
  const mappings = {};
  for (const table of IMPORT_ORDER) mappings[table] = new Map();
  for (const row of data || []) {
    if (mappings[row.source_table]) mappings[row.source_table].set(row.source_id, row.destination_id);
  }
  return mappings;
}

function retainLiveMappings(mappings, indexes) {
  const liveMappings = {};
  for (const table of IMPORT_ORDER) {
    liveMappings[table] = new Map();
    for (const [sourceId, destinationId] of mappings[table] || []) {
      if (indexes[table]?.byId.has(destinationId)) liveMappings[table].set(sourceId, destinationId);
    }
  }
  return liveMappings;
}

function findExistingDestination(table, sourceRow, payload, indexes, sameProfile) {
  if (sameProfile) {
    const sameId = indexes[table]?.byId.get(sourceRow.id);
    if (sameId) return { destinationId: sameId, reason: 'same_profile' };
  }
  const duplicateId = indexes[table]?.byFingerprint.get(recordFingerprint(payload));
  return duplicateId ? { destinationId: duplicateId, reason: 'content_match' } : null;
}

export async function restoreProfileBackup(supabase, user, buffer, { dryRun = false } = {}) {
  if (!user?.id) throw new Error('You must be signed in.');
  const inspected = inspectProfileBackup(buffer);
  assertArchiveOwner(inspected.manifest, inspected.preferences, user.id);
  const sameProfile = true;
  const report = { inserted: 0, skipped: 0, duplicates: 0, conflicts: [], tables: {}, assets: 0 };
  const [indexes, savedMappings] = await Promise.all([
    loadExistingIndexes(supabase, user.id),
    loadExistingMappings(supabase, user.id),
  ]);
  const priorMappings = retainLiveMappings(savedMappings, indexes);
  const mappings = {};
  for (const table of IMPORT_ORDER) mappings[table] = new Map();
  ...
  if (dryRun) {
    for (const table of IMPORT_ORDER) {
      ...
      for (const row of rows) {
        ...
        const mapped = priorMappings[table].get(row.id) || null;
        const payload = setForeignKeys(table, row, mappings);
        const existing = mapped ? { destinationId: mapped, reason: 'prior_import' } : findExistingDestination(table, row, payload, indexes, sameProfile);
        if (existing) {
          mappings[table].set(row.id, existing.destinationId);
          duplicates += 1;
        }
      }
      report.tables[table] = { proposed: rows.length - duplicates, duplicates };
      report.duplicates += duplicates;
    }
    return { report, manifest: inspected.manifest };
  }

  const urlMap = await restoreAssets(supabase, user.id, inspected, sameProfile);
  report.assets = urlMap.size;
  for (const table of IMPORT_ORDER) {
    const rows = inspected.tables[table] || [];
    const pendingMappings = [];
    report.tables[table] = { inserted: 0, skipped: 0, duplicates: 0 };
    for (const row of rows) {
      if (!row?.id || typeof row.id !== 'string') throw new Error(`Backup ${table} contains an invalid source id.`);
      const mapped = priorMappings[table].get(row.id) || null;
      const basePayload = setForeignKeys(table, row, mappings);
      const existing = mapped ? { destinationId: mapped, reason: 'prior_import' } : findExistingDestination(table, row, basePayload, indexes, sameProfile);
      if (existing) {
        mappings[table].set(row.id, existing.destinationId);
        pendingMappings.push({ user_id: user.id, source_table: table, source_id: row.id, destination_id: existing.destinationId });
        report.skipped += 1; report.duplicates += 1;
        report.tables[table].skipped += 1; report.tables[table].duplicates += 1;
        continue;
      }
      const payload = applyAssetUrlMap(basePayload, urlMap);
      ...
      pendingMappings.push({ user_id: user.id, source_table: table, source_id: row.id, destination_id: data.id });
      mappings[table].set(row.id, data.id);
      indexes[table].byId.set(data.id, data.id);
      indexes[table].byFingerprint.set(recordFingerprint(payload), data.id);
      report.inserted += 1; report.tables[table].inserted += 1;
    }
    await saveMappings(supabase, pendingMappings);
  }

...
async function loadExistingIndexes(supabase, userId) {
  ...
  for (const spec of TABLE_SPECS) {
    ...
    indexes[spec.file] = { byId, byFingerprint };
  }
  return indexes;
}
...
export async function restoreProfileBackup(supabase, user, buffer, { dryRun = false } = {}) {
  ...
  const indexes = await loadExistingIndexes(supabase, user.id);
  const mappings = {};
  ...
  if (dryRun) {
    for (const table of IMPORT_ORDER) {
      ...
      for (const row of rows) {
        ...
        const mapped = await findExistingBySourceId(supabase, table, row.id, user.id);

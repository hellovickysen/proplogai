 ...
 */
export async function createSubscription({ billingCycle, email, userId, offerId = null }) {
  ...
  // 14-day trial: set start_at to 14 days from now
  const trialEnd = Math.floor(Date.now() / 1000) + 14 * 24 * 60 * 60;

  // Callback URL for hosted pages is set in Razorpay Dashboard,
  // not in the API payload (callback_url/callback_method are rejected).

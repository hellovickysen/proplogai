 ...
 * Optional: RAZORPAY_OFFER_ID_5 (partner discount offer, created in Dashboard)
 */
 ...
 * @param {string} [options.offerId] - optional Razorpay offer id (e.g. partner discount)
 ...
 * @param {number} [options.startAt] - optional explicit first-charge time (unix seconds).
 *        When provided it wins over `trial`. Used to preserve a user's remaining trial
 *        days when they convert to a discounted plan mid-trial: the first (discounted)
 *        charge is pinned to their existing trial end date.
 * @returns {Promise<object>} Razorpay subscription object
 */
export async function createSubscription({ billingCycle, email, userId, offerId = null, trial = true, startAt = null }) {
  ...
  // First-charge timing:
  // - explicit startAt (unix seconds) wins — pins the first charge to a fixed
  //   date, used to preserve remaining trial days on a mid-trial conversion.
  // - else a 14-day trial defers the first charge 14 days out.
  // - else (discounted immediate checkout) omit start_at so the first cycle
  //   charges right away.
  if (startAt) {
    body.start_at = Math.floor(startAt);
  } else if (trial) {
    body.start_at = Math.floor(Date.now() / 1000) + 14 * 24 * 60 * 60;
  }

  // Attach a discount offer (e.g. partner 30%) when provided.

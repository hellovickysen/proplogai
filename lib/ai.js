...
// Set OPENROUTER_MODEL in production to override the default model slug.
// The correct OpenRouter slug for Claude 3.5 Haiku is 'anthropic/claude-3-5-haiku'.
function model() {
...
function tradeToText(trade, journal) {
  ...
  if (journal) {
    ...
    if (journal.note) {
      L.push('Trader note: <trader_note>' + String(journal.note).slice(0, 160) + '</trader_note>');
    } else {
      L.push('Trader note: (none)');
    }
  } else {
...
IMPORTANT: Include a "rulebook_discipline" section analyzing setup adherence. If the trader has setup discipline data (setup_followed, no_setup_reason), analyze:
- What % of trades followed their setup vs partially/not followed
- Whether "No Setup" trades cluster after losses (revenge pattern)
- Which setups have the best/worst win rates
- Correlation between setup adherence and profitability
...
Respond with ONLY valid JSON (no markdown) matching exactly:
{
  ...
  "rulebook_discipline": {
    ...
    "recommendation": "<one actionable rule to improve discipline>"
  }
}`;

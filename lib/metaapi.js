...
// Read the live MetaApi account state + broker connection status.
export async function getAccountStatus(accountId) {
  try {
    ...
    if (!res.ok) return { ok: false, state: null, connectionStatus: null };
    const data = await res.json();
    return { ok: true, state: data.state || null, connectionStatus: data.connectionStatus || null };
  } catch (e) {
    return { ok: false, state: null, connectionStatus: null };
  }
}

...
export async function backfillEvaluations(supabase, userId, accountId) {
  let query = supabase
    .from('trades')
    .select('id, account_id, trade_date, pnl, lot_size, stop_loss, entry_price, exit_price, direction, setup_ids, setup_follow_map, setup_followed, no_setup_reason, bad_sl, session, pair')
    .eq('user_id', userId)
  ...
  if (tradesErr) return { total: 0, evaluated: 0, errors: [tradesErr.message] };

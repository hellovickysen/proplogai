...
export async function evaluateTrade(supabase, userId, trade, journal, setupNameMap) {
  ...
  // -- Over-trading (daily trade count limit). The Rulebook UI stores this as
  //    'maximum_trades_per_day'; the evaluation is emitted under 'over_trading'. --
  const overTradingRule = applicable.find((r) => r.rule_key === 'maximum_trades_per_day');
...
function evaluateRiskPerTrade(trade, rule) {
  ...
  // Percent-based risk needs account size to convert to a dollar loss; not supported for auto-evaluation yet.
  if (rule.unit === '%') {
    return { rule_key: 'risk_per_trade', rule_label: rule.title || 'Risk Per Trade', outcome: 'unknown', evidence: { reason: 'Percent-based risk not yet supported — set this limit in $ to track it.' }, rule_snapshot: ruleSnapshot(rule) };
  }

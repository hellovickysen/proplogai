 ...
 * partnerPct / trialAutoPct are the editable rates; trialAutoCode is the display
 * name for the trial auto-bonus at checkout (e.g. "TRIAL10"); the *OfferUpi
 * values are the pre-made Razorpay UPI offer ids for each scenario (charge-now):
...
export async function getDiscountSettings(adminSb) {
  ...
  try {
    ...
    return {
      ...
      trialAutoCode: (m.trial_auto_code || 'TRIAL10').toString().trim().toUpperCase().slice(0, 20) || 'TRIAL10',

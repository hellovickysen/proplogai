 ...
 * Every new signup is Basic. Opting into the 14-day trial grants full Elite
 * access for the trial window (shown as a "Trial" badge with a countdown).
 * Paid subscribers are Elite. Admin (ADMIN_EMAIL) has unrestricted access.
 *
...
export async function getUserAccess(supabase, user) {
  ...
  const effectivePlan = isAdmin || (plan === 'elite' && (isActiveSub || isTrialing))

 ...
 * Every signup auto-gets a 14-day trial (no card required) with full Elite
 * access, shown as a "Trial" badge with a countdown. When the trial lapses the
 * account drops to Basic. Paid subscribers are Elite. Admin (ADMIN_EMAIL) has
 * unrestricted access.
 *
 * Access is driven purely by the subscription row:
 * - a real paid sub (has a Razorpay id AND status active/authenticated) → Elite
 * - a future trial_ends_at → Elite (DB-only, no-card trial), regardless of plan
 * - otherwise → Basic (includes lapsed trials)
 *
...
export const FEATURES = {
  ...
  profile_backup: {
    label: 'Whole-profile backup and restore',
    basic: true,
    elite: true,
  },
  google_drive_backup: {
    label: 'Daily Google Drive backup',
    basic: false,
    elite: true,
  },
  shareable_cards: {
...
export const ELITE_FEATURES = [
  ...
  { icon: '🏷️', label: 'Unlimited tags & emotions' },
];
...
export async function getUserAccess(supabase, user) {
  ...
  const isAdmin = prefs?.is_admin === true || user.email === ADMIN_EMAIL;
  ...
  // Elite when: admin, OR in an active (DB-only, no-card) trial, OR a real paid
  // subscription (Razorpay id present and active/authenticated). Otherwise Basic.
  const hasRazorpaySub = !!sub?.razorpay_subscription_id;
  const isActiveSub = hasRazorpaySub && (sub.status === 'active' || sub.status === 'authenticated');
  const isTrialing = !!(sub?.trial_ends_at && new Date(sub.trial_ends_at) > new Date());
  const elite = isAdmin || isTrialing || isActiveSub;

  return buildAccess(elite ? 'elite' : 'basic', isBeta, isAdmin, {
...
export function buildAccess(plan, isBeta, isAdmin, subInfo = {}) {
  // Effective plan: admin or an elite plan gets elite access
  const effectivePlan = isAdmin || plan === 'elite' ? 'elite' : 'basic';
  ...
  return {
     ...
     * Admin/elite: always true. Basic: depends on feature.
     */
    canUse(feature) {
    ...
    /**
     * Legacy hook — beta warnings were removed. Always false now.
     */
    showWarning(feature) {
      return false;
    },

...
export function buildAccess(plan, isBeta, isAdmin) {
  ...
  return {
     ...
     * Returns -1 for unlimited, false for blocked boolean features.
     * Never returns Infinity (not JSON-serializable for Next.js server→client props).
     */
    limit(feature) {
      const f = FEATURES[feature];
      if (!f) return -1;
      if (isAdmin) return -1;
      const val = f[effectivePlan] ?? f.basic;
    ...
    async remaining(feature, supabase, userId) {
      const f = FEATURES[feature];
      if (!f) return { used: 0, limit: -1, remaining: -1 };

      const lim = this.limit(feature);
      if (lim === -1 || lim === true) return { used: 0, limit: -1, remaining: -1 };

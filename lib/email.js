...
export function buildCoachReportEmail(report, stats) {
  ...
  const insightRows = insights.map((i) => `
    <li style="margin-bottom:6px;color:#d0d0d0;">
      <strong style="color:#ffc42d;">${i.emotion || ''}</strong>: ${i.stat || ''} — ${i.observation || ''}
    </li>
  `).join('');

  const guardrailRows = guardrails.map((g) => `
    <li style="margin-bottom:4px;color:#ff9f1c;">✓ ${g}</li>
  `).join('');
...
<body style="margin:0;padding:0;background:#07070b;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:32px 20px;">
    <div style="text-align:center;margin-bottom:24px;">
      <span style="font-size:24px;font-weight:700;background:linear-gradient(135deg,#ffc42d,#ff9f1c);-webkit-background-clip:text;-webkit-text-fill-color:transparent;">PropJournal</span>
    </div>
    ...
    <div style="text-align:center;padding:16px 0;">
      <a href="https://pipmind-sigma.vercel.app/dashboard/coach" style="display:inline-block;padding:10px 24px;background:linear-gradient(135deg,#ffc42d,#ff9f1c);color:#08080f;font-weight:600;font-size:14px;border-radius:10px;text-decoration:none;">View Full Report</a>
    </div>

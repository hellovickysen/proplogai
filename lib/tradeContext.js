...
export async function getUserTradeContext(supabase, userId, opts = {}) {
  ...
  const { data: recentTrades } = await supabase
    .from('trades')
    .select('pair, direction, pnl, r_multiple, setup, setup_followed, session, timeframe, trade_date')
    .eq('user_id', userId)
  ...
  if (trades.length > 0) {
    ...
    const avgR = trades.filter((t) => t.r_multiple != null).reduce((a, t) => a + Number(t.r_multiple), 0) / (trades.filter((t) => t.r_multiple != null).length || 1);
